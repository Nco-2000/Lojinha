        ...
        if (current_order.Status !== "Awaiting Confirmation") {
            ...
            const listaDados = await Promise.all(itemOrders.map(async (itemOrder) => {
                ...
                if(currentProduct !== null && currentProduct !== undefined && currentProduct !== NaN){
                    Price_Order += itemOrder.Price_total
                    return {
            ...
            return res.render('Orders/Cart', {listaDados, success : true, Price_Order});
        }
    }
});
...
router.delete('/Remove/:ID_Product', verifyToken, async(req,res)=>{
    const {ID_Product} = req.params
    ...
    const current_order = await Order.findByPk(client.Current_Cart_id);
    console.log(current_order.ID_ORDER)

    ItemOrder.destroy({where: {Product_id : ID_Product, Order_id : current_order.ID_ORDER}}).then(deleted => {
        if (deleted) {
            res.redirect('Orders/cart')
        } else {
            res.send('<h1>The product is not in the cart</h1>');
        }
      }).catch(error => {
        console.log(error)
        res.send('<h1>Error deleting product from the cart</h1>');
      });
